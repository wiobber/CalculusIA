...
\begin{question}
  ...
  \end{question}
  \begin{explanation}
      If $f(x)$ is invertible at $x=1$, that means that we want $f^{-1}(1)$ to exist. This means that we want $f(x)=1$ for a single $x$-value. Using the graph we can see that this occurs when $x=1.5$. Thus, $f^{-1}(1)=1.5$. 
  \end{explanation}
  \end{prompt}
\end{question}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    Celsius measurements for temperature.
...
\begin{question}
  ...
  \begin{feedback}
    ...
    7$.  Since $f(2)=3$ and $f(7)=3$ we have both $f^{-1}(3) = 2$ and $f^{-1}(3) = 7$.  Since
...
\end{selectAll}
\begin{feedback}
    In each case, we want to consider if, for each $y$-value, is there exactly one $x$-value that maps to it. If the answer is yes, then the function in one-to-one.
\end{feedback}
\end{question}
...
vertical line crosses the plot at most once. We called this
